const state={sites:[],sections:[],category:"الكل",tabs:[],activeTab:null};
...
async function init(){
  ...
  try{
    ...
    if(Array.isArray(data)){state.sites=data;state.sections=[]}
    else{state.sites=data.courses||data.sites||[];state.sections=data.sections||[]}
...
function collectAssetUrls(){
  const urls=new Set();
  state.sites.forEach(s=>{(s.parts||[]).forEach(p=>{if(p.file)urls.add(resolve(p.file))});if(s.image)urls.add(resolve(s.image))});
  return [...urls];
}
async function setupOfflineCache(){
  ...
  try{
    ...
    const send=()=>{if(navigator.serviceWorker.controller)navigator.serviceWorker.controller.postMessage({type:"PRECACHE_URLS",urls:collectAssetUrls()})};
    send();
    navigator.serviceWorker.addEventListener("controllerchange",send);
  }catch(e){}
}
function allCategories(){if(state.sections.length)return state.sections.map(s=>s.name).filter(Boolean);return [...new Set(state.sites.flatMap(s=>s.category?[s.category]:[]))].filter(Boolean)}
